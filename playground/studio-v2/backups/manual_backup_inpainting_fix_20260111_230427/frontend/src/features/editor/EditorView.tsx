...
export const EditorView: React.FC<EditorViewProps> = ({
    ...
    // 5. LIMPAR IMAGEM (Inpainting) - CORRIGIDA
    const handleCleanImage = async () => {
        ...
        try {
            ...
            const cleanPayload = balloons.map(b => {
                ...
                // 1. Tenta pegar posição visual (Konva) ou Fallback para box_2d
                // box_2d padrão no frontend é [y, x, y+h, x+w] (pixels)
                let bx = anyB.x;
                let by = anyB.y;
                let bw = anyB.width;
                let bh = anyB.height;
                // Se faltar dados visuais, recupera do box_2d original
                if (bx === undefined || bw === undefined) {
                    if (b.box_2d && b.box_2d.length === 4) {
                        const [y1, x1, y2, x2] = b.box_2d;
                        bx = x1;
                        by = y1;
                        bw = x2 - x1; // Calcula largura
                        bh = y2 - y1; // Calcula altura
                    } else {
                        // Último caso: ignora balão inválido
                        return null;
                    }
                }
                // Aplica escala se houver (editado pelo usuário)
                const scaleX = anyB.scaleX || 1;
                const scaleY = anyB.scaleY || 1;
                const finalW = bw * scaleX;
                const finalH = bh * scaleY;
                // 2. NORMALIZAÇÃO (0-1000)
                // Regra de três simples: (Pixel / Total) * 1000
                ...
                return {
                    ...b,
                    // Payload para o Backend: [ymin, xmin, ymax, xmax]
