...
async def lifespan(app: FastAPI):
    ...
    """
    # STARTUP: Auto-cleanup of old temp files
    print("🧹 Running auto-cleanup of old temporary files...")
    ...
    try:
        ...
        # Clean image files older than 24 hours
        ...
        if deleted_count > 0:
            ...
            print(f"✅ Deleted {deleted_count} old temp files ({freed_mb:.1f} MB freed)")
        else:
            print("✅ No old temp files to clean")
            
    except Exception as e:
        print(f"⚠️  Error during temp cleanup: {e}")
    
    # Yield control to the application
    yield
    
    # SHUTDOWN: Future cleanup logic can go here
    print("👋 Shutting down Imagine Read Engine...")
...
# --- DATA MANAGEMENT (data.json contains complete state) ---
DATA_FILE = os.path.join(BASE_DIR, "data.json")

def load_data() -> dict:
    """Load complete application data from data.json"""
    if not os.path.exists(DATA_FILE):
        return {"projects": [], "fileSystem": []}
    try:
...
def save_data(data: dict):
    """Save complete application data to data.json"""
...
@app.get("/projects")
def get_projects():
    """Get all projects from data.json"""
    data = load_data()
    return data.get("projects", [])

@app.get("/filesystem")
def get_filesystem():
    """Get filesystem from data.json"""
    data = load_data()
    return data.get("fileSystem", [])

@app.post("/projects")
def create_project(data_input: dict):
    """Create a new project"""
    data = load_data()
    projects = data.get("projects", [])
    
    root_folder_id = f"folder-{int(datetime.now().timestamp() * 1000)}-root"
    new_project = {
...
def update_project(project_id: str, data_input: dict):
    ...
    if "name" in data_input:
        projects[project_index]["name"] = data_input["name"]
    if "color" in data_input:
        projects[project_index]["color"] = data_input["color"]
    if "isPinned" in data_input:
        projects[project_index]["isPinned"] = data_input["isPinned"]
    
    projects[project_index]["lastModified"] = datetime.now().isoformat()
...
async def clean_page(request: CleanRequest):
    try:
        ...
        with PILImage.open(local_image_path) as img:
            ...
            # 3. Single AI Call (Full Image + Full Mask) with Google Gen AI SDK
            ...
            try:
                ...
                # 4. Smart Compositing (The "Zero Distortion" Fix)
...
# NOTE: Removed legacy function executar_ocr_nos_baloes() (unused - replaced by Vertex AI OCR)
